	...
	d3.select(sectionObj).selectAll("ol>li").each(function(d, i)
		{
			...
			else if (d.getCells())
			{
				...
				{
					var command;
					command = {containerUUID: d.cell.parent.id(), 
...
cr.ObjectCell.prototype.show = function(obj, backText)
{
	...
	if (this.isUnique())
	{
		if (!_isPickCell(this))
			sectionObj.classed("btn row-button", true)
			          .on("click", function(cell) {
				if (prepareClick('click', 'view unique ' + cell.field.name + ': ' + cell.data[0].description()))
				{
					try {
	...
	if (_isPickCell(this) || this.isUnique())	/* Unique value handles the click above */
		...
		clickFunction = function(d) {
			if (prepareClick('click', 'view multiple ' + d.cell.field.name + ': ' + d.description()))
			{
				try {
...
cr.ObjectCell.prototype.showEdit = function(obj, backText)
{
	...
	if (!this.isUnique())
	{
		...
		crv.appendAddButton(sectionObj, this.field.name, done);
		sitePanel.setupItemsDivHandlers(itemsDiv, this);
	}
}
