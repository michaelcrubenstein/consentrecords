...
data = showData('access source/%s' % userID, context, GrantTarget)
userAccessID = data[0]['user accesses'][0]['id']
groupAccessID = data[0]['group accesses'][0]['id']
data = showData('user access/%s' % userAccessID, context, UserGrant)
data = showData('group access/%s' % groupAccessID, context, GroupGrant)

data = showData('access source/%s' % userID, anoncontext, GrantTarget)
data = showData('access source/%s' % userID, escontext, GrantTarget)
